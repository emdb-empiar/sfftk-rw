...
*   the **adapter** API (:py:mod:`sfftkrw.schema.adapter`) which handles how data fields are represented independent of
...
All aspects of the structure of an EMDB-SFF file are handled by :py:mod:`sfftkrw.schema.adapter` package which
...
.. code-block:: python
    ...
    seg_fn
    '/Users/pkorir/PycharmProjects/sfftk-rw/sfftkrw/test_data/sff/v0.7/emd_1014.sff'
...
.. code-block:: python
    ...
    print(segment.biological_annotation.numberOfInstances)
    1
    
    print(segment.biological_annotation.externalReferences)
    External references list with 2 reference(s)
    
    print(segment.biological_annotation.externalReferences[0]) # first reference
...
.. code-block:: python
    ...
    # bounding box
    seg.bounding_box = schema.SFFBoundingBox(
    ...     xmin=0,
    ...     xmax=512,
    ...     ymin=0,
    ...     ymax=1024,
...
.. code-block:: python
    ...
    extRefs.add_external_reference(
...
* first, we need to define the volumes that contain the actual volume data as a set of :py:class:`sfftkrw.schema.adapter.SFFLattice` objects contained in a :py:class:`sfftkrw.schema.adapter.SFFLatticeList`;
